...
✅ UPDATED FOR FAMILY ACCOUNTS FEATURE
-------------------------------------------
Changes made:
...
from sqlalchemy.orm import sessionmaker, Session, relationship as db_relationship
...
class User(Base):
    ...
    zip_code = Column(String, nullable=True)  # 🔧 FIX #4: ZIP CODE FOR LOCAL SERVICES
    ...
    # Relationships
    conversations = db_relationship("Conversation", back_populates="user", cascade="all, delete-orphan")
    health_profile = db_relationship("HealthProfile", back_populates="user", uselist=False, cascade="all, delete-orphan")
    symptoms = db_relationship("Symptom", back_populates="user", cascade="all, delete-orphan")
    treatments = db_relationship("Treatment", back_populates="user", cascade="all, delete-orphan")
    family_members = db_relationship("FamilyMember", back_populates="user", cascade="all, delete-orphan")
    custom_protocols = db_relationship("CustomProtocol", back_populates="user", cascade="all, delete-orphan")
...
class Conversation(Base):
    ...
    # Relationships
    user = db_relationship("User", back_populates="conversations")
...
class HealthProfile(Base):
    ...
    blood_type = Column(String, nullable=True)  # ✅ NEW FIELD
    ...
    # Relationships
    user = db_relationship("User", back_populates="health_profile")
    family_member = db_relationship("FamilyMember", back_populates="health_profile", foreign_keys=[family_member_id], uselist=False)  # ✅ Specify foreign key
...
class Symptom(Base):
    ...
    # Relationships
    user = db_relationship("User", back_populates="symptoms")
...
class Treatment(Base):
    ...
    # Relationships
    user = db_relationship("User", back_populates="treatments")
...
class FamilyMember(Base):
    ...
    # ✅ UPDATED: String ID to match main.py (uses uuid.uuid4() as string)
    id = Column(String, primary_key=True, index=True)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    
    # Basic info
    name = Column(String, nullable=False)
    relationship = Column(String, nullable=False)  # ✅ RENAMED from relation_type to match API
    age = Column(Integer, nullable=True)
    date_of_birth = Column(Date, nullable=True)  # ✅ NEW FIELD
    gender = Column(String, nullable=True)
    notes = Column(Text, nullable=True)
    
    # Health profile reference (NOT a foreign key - just stores the ID for convenience)
    # The actual relationship is via HealthProfile.family_member_id
    health_profile_id = Column(String, nullable=True)
    
    # Tracking
    conversation_count = Column(Integer, default=0)  # ✅ NEW FIELD
    created_at = Column(DateTime, default=datetime.utcnow)
    ...
    # ONE-TO-ONE relationship via HealthProfile.family_member_id
    health_profile = db_relationship("HealthProfile", back_populates="family_member", uselist=False)
...
class CustomProtocol(Base):
    ...
    # Relationships
    user = db_relationship("User", back_populates="custom_protocols")
        ...
        with engine.connect() as conn:
            ...
            if 'messages' in inspector.get_table_names():
                try:
                    ...
                    # If column_default is NULL, the ID doesn't autoincrement - DROP IT!
                    if row and row[0] is None:
                        print("  ⚠️  messages.id is broken (no autoincrement)")
                        print("  🗑️  Dropping messages table...")
                        conn.execute(text("DROP TABLE messages CASCADE;"))
                        conn.commit()
                        print("  ✅ Broken table dropped - will be recreated")
                    else:
                        print("  ✅ messages table is OK")
                except Exception as e:
                    print(f"  ℹ️  Could not check messages: {e}")
            else:
                print("  ℹ️  messages doesn't exist yet (will be created)")
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if 'users' in inspector.get_table_names():
                ...
                required_columns = {
                    'name': 'VARCHAR',
                    'zip_code': 'VARCHAR',  # 🔧 FIX #4: ZIP CODE
...
def create_user(db: Session, email: str, password_hash: str, name: str = None, zip_code: str = None) -> User:
    """Create new user"""
    user = User(
        email=email,
        password_hash=password_hash,
        name=name,
        zip_code=zip_code,  # 🔧 FIX #4
        is_active=True
    )
    db.add(user)
    db.commit()
