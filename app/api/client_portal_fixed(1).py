...
async def generate_client_link(
    ...
    try:
        ...
        if not client_id:
            raise HTTPException(
                status_code=422, 
                detail="Missing client_id or clientId in request body"
            )
...
@router.get("/client-view/{token}/verify")
async def verify_client_token(token: str):
    ...
    with engine.connect() as conn:
        ...
        return {
            ...
            "client_id": row[1],
            "client_name": row[4],
            "practitioner_name": row[5]
        }


@router.get("/client-view/{token}/replies")
async def get_client_replies(token: str):
    """Get replies for a client (placeholder - integrate with messages)"""
    with engine.connect() as conn:
        ...
        # Return empty replies for now - integrate with client_messages later
        return {
            "replies": [],
            "client_id": row[0]
        }
...
@router.delete("/client-view/revoke/{token}")
...
@router.get("/client-view/active-links")
