...
async def get_client_compliance_details(member_id: int, current_user: dict = Depends(get_current_user)):
    ...
    with engine.connect() as conn:
        ...
        # Get active protocol
        protocol = conn.execute(text("""
            SELECT cp.id, cp.current_week, p.name as protocol_name, p.total_weeks
        ...
        category_breakdown = {}
        if compliance_details and compliance_details.get('completed_items'):
            completed_ids = {item['id'] for item in compliance_details['completed_items']}
            incomplete_ids = {item['id'] for item in compliance_details['incomplete_items']}
            ...
            for item in compliance_details['completed_items']:
                category = item['id'].split('-')[0]
                if category in categories:
                    categories[category]['completed'] += 1
                    categories[category]['total'] += 1
            
            for item in compliance_details['incomplete_items']:
                category = item['id'].split('-')[0]
                if category in categories:
                    categories[category]['total'] += 1
        ...
        return {
            ...
            "logged_at": compliance_log[4].isoformat() if compliance_log[4] else None,
            "completed_items": compliance_details.get('completed_items', []) if compliance_details else [],
            "incomplete_items": compliance_details.get('incomplete_items', []) if compliance_details else [],
