...
class Message(Base):
    ...
    image_data = Column(JSON, nullable=True)  # ✅ NEW: Store image data for persistence
...
def add_message(db: Session, conversation_id: str, role: str, content: str, image_data: dict = None) -> Message:
    """Add message to conversation with optional image data"""
    # Convert string UUID to UUID object if needed
    if isinstance(conversation_id, str):
        conversation_id = uuid.UUID(conversation_id)
    ...
    message = Message(
        ...
        image_data=image_data  # ✅ NEW: Store image data
    )
    db.add(message)
