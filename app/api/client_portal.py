            ...
            if isinstance(nut_data, dict):
                ...
                if nut_data.get('foods_to_include'):
                    ...
                    if isinstance(foods, list) and foods:
                        ...
                        if foods_str:
                            lifestyle_changes.append(f"✅ Foods to Include: {foods_str}")
                        ...
                        if foods_list:
                            lifestyle_changes.append(f"✅ Foods to Include: {', '.join(foods_list)}")
                ...
                if nut_data.get('foods_to_avoid'):
                    ...
                    if isinstance(foods, list) and foods:
                        foods_str = ', '.join(str(f).strip() for f in foods if f and str(f).strip())
                        if foods_str:
                            lifestyle_changes.append(f"⛔ Foods to Avoid: {foods_str}")
                    elif isinstance(foods, str) and foods.strip():
                        # Split by common delimiters if it's a string
                        foods_list = [f.strip() for f in foods.replace(',', '\n').replace(';', '\n').split('\n') if f.strip()]
                        if foods_list:
                            lifestyle_changes.append(f"⛔ Foods to Avoid: {', '.join(foods_list)}")
                
                # Add meal timing/structure
                if nut_data.get('meal_timing') and str(nut_data['meal_timing']).strip():
                    lifestyle_changes.append(f"⏰ Meal Timing: {nut_data['meal_timing']}")
                elif nut_data.get('meal_structure') and str(nut_data['meal_structure']).strip():
                    lifestyle_changes.append(f"⏰ Meal Structure: {nut_data['meal_structure']}")
                
                # Add hydration goals
                if nut_data.get('hydration') and str(nut_data['hydration']).strip():
                    lifestyle_changes.append(f"💧 Hydration: {nut_data['hydration']}")
                elif nut_data.get('hydration_goals') and str(nut_data['hydration_goals']).strip():
                    lifestyle_changes.append(f"💧 Hydration: {nut_data['hydration_goals']}")
                
                # Add daily calories if present
                if nut_data.get('daily_calories') and str(nut_data['daily_calories']).strip():
                    lifestyle_changes.append(f"📊 Daily Calories: {nut_data['daily_calories']}")
                    
            elif isinstance(nut_data, str) and nut_data.strip():
                lifestyle_changes.append(f"🥗 Nutrition: {nut_data}")
        ...
        if protocol[10]:  # weekly_notes
            ...
            if isinstance(notes_data, dict):
                ...
                if current_week_num in week_notes:
                    lifestyle_changes.insert(0, f"⭐ This Week's Focus: {week_notes[current_week_num]}")
                ...
                if len(week_notes) > 0:
                    ...
                    if timeline_items:
                        ...
                        lifestyle_changes.insert(1 if current_week_num in week_notes else 0,
                                                "📅 ─── Protocol Timeline ───")
                        for idx, item in enumerate(timeline_items):
                            lifestyle_changes.insert((2 if current_week_num in week_notes else 1) + idx, 
                                                    f"   {item}")
                        
            elif isinstance(notes_data, str) and notes_data.strip():
                # If weekly_notes is just a string, use it as the focus
                lifestyle_changes.insert(0, f"⭐ This Week's Focus: {notes_data}")
