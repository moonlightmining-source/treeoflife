...
✅ INCLUDES FAMILY MEMBERS ENDPOINTS
"""
...
async def get_health_profile(request: Request, member_id: Optional[str] = None):
    """Get user's health profile or family member's profile"""
    user_id = get_current_user_id(request)
    ...
    with get_db_context() as db:
        if member_id:
            ...
            # Get or create profile for family member
...
async def update_profile(profile_update: HealthProfileUpdate, request: Request, member_id: Optional[str] = None):
    """Update user's health profile or family member's profile"""
    user_id = get_current_user_id(request)
...
async def get_family_members(request: Request):
    ...
    with get_db_context() as db:
        ...
        return {
            "members": [
                {
                    ...
                    "notes": member.notes,
                    "health_profile_id": member.health_profile_id,
                    "conversation_count": member.conversation_count or 0,
            ...
            "limit": FAMILY_MEMBER_LIMITS.get(user.subscription_tier, 0),
            "count": len(members)
        }
...
async def create_family_member(member_data: FamilyMemberCreate, request: Request):
    ...
    with get_db_context() as db:
        ...
        if current_count >= limit:
            raise HTTPException(
                status_code=403,
                detail=f"Family member limit reached for {user.subscription_tier} tier. Upgrade to add more members."
            )
