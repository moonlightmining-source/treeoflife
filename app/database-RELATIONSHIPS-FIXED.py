...
class User(Base):
    ...
    location = Column(String, nullable=True)        # ✅ NEW: Location field
...
class HealthProfile(Base):
    ...
    # Foreign key is on this table (family_member_id)
    family_member = relationship("FamilyMember", back_populates="health_profile", uselist=False)
...
class FamilyMember(Base):
    ...
    # ✅ String ID to match main.py (uses uuid.uuid4() as string)
    id = Column(String, primary_key=True, index=True)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    
    # Basic info
    name = Column(String, nullable=False)
    relation = Column('relationship', String, nullable=False)  # ✅ Maps to 'relationship' in DB, called 'relation' in code
    age = Column(Integer, nullable=True)
    date_of_birth = Column(Date, nullable=True)
    gender = Column(String, nullable=True)
    notes = Column(Text, nullable=True)
    ...
    # Relationships (SQLAlchemy, not a column)
    user = relationship("User", back_populates="family_members")
    # Back reference - HealthProfile has the FK to us
    health_profile = relationship("HealthProfile", back_populates="family_member", uselist=False)
    
    # ✅ Property for backward compatibility (main.py uses .relationship)
    @property  
    def relationship(self):
        """Alias for relation field (for backward compatibility)"""
        return self.relation
    
    # ✅ Property for health_profile_id (computed from relationship)
    @property
    def health_profile_id(self):
        """Get health profile ID from relationship"""
        return self.health_profile.id if self.health_profile else None
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if 'users' in inspector.get_table_names():
                ...
                # ===================================================================
                # NUCLEAR OPTION: DROP NOT NULL FROM ALL USER COLUMNS
                # (Except critical ones: id, email, password_hash, created_at, is_active)
                # ===================================================================
                print("  🔧 NUCLEAR FIX: Making ALL user columns nullable...")
                
                # Get ALL columns in users table
                all_user_columns = [col['name'] for col in inspector.get_columns('users')]
                
                # Critical columns that MUST stay NOT NULL
                critical_columns = ['id', 'email', 'password_hash', 'created_at', 'is_active']
                
                # Drop NOT NULL from everything else
                fixed_count = 0
                for col_name in all_user_columns:
                    if col_name not in critical_columns:
                        try:
                            conn.execute(text(f"""
                                ALTER TABLE users ALTER COLUMN {col_name} DROP NOT NULL;
                            """))
                            fixed_count += 1
                            print(f"  ✅ Made '{col_name}' nullable")
                        except Exception as e:
                            error_msg = str(e).lower()
                            # Ignore "column does not exist" and "already nullable" errors
                            if 'does not exist' not in error_msg and 'not-null' not in error_msg:
                                print(f"  ℹ️  {col_name}: {e}")
                
                if fixed_count > 0:
                    conn.commit()
                    print(f"  🎉 Made {fixed_count} columns nullable!")
                else:
                    print("  ℹ️  All columns already nullable")
            else:
...
def create_user(db: Session, email: str, password_hash: str, name: str = None, zip_code: str = None) -> User:
    """Create new user - splits full name into first_name and last_name"""
    # Split name into first and last
    if name:
        name_parts = name.strip().split(' ', 1)  # Split on first space only
    ...
    user = User(
        ...
        last_name=last_name,
        name=name or f"{first_name} {last_name}".strip(),  # Store full name too
        zip_code=zip_code,
        is_active=True
    )
    db.add(user)
