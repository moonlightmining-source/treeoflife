...
Auth + Chat functionality with AI Personalization
"""
...
def build_personalized_prompt(user_name: str, email: str = None) -> str:
    """Build a personalized system prompt with user's name"""
    
    # Get user's health profile if available
    profile = health_profiles.get(email, {}) if email else {}
...
You provide guidance from 11 medical traditions:
1. Western Medicine
2. Ayurveda
...
11. Consciousness Mapping (Tarot, Astrology, I Ching, Hero's Journey)"""
    ...
    if profile:
        prompt += f"\n\nPERSONALIZATION FOR {user_name.upper()}:"
        if profile.get('ayurvedic_dosha'):
            prompt += f"\n- Ayurvedic Constitution: {profile['ayurvedic_dosha']}"
        if profile.get('health_goals'):
            prompt += f"\n- Health Goals: {', '.join(profile['health_goals'])}"
        if profile.get('preferred_traditions'):
            prompt += f"\n- Preferred Traditions: {', '.join(profile['preferred_traditions'])}"
...
- Be empowering rather than prescriptive
...
- If {user_name} describes emergency symptoms (chest pain, severe bleeding, loss of consciousness, 
  severe allergic reaction, suicidal thoughts), respond with:
  "🚨 {user_name}, this sounds like a medical emergency. Please call 911 or go to the nearest emergency 
...
Remember: You're having a conversation with {user_name}, not giving a lecture. Be personable and supportive!"""
...
def root():
    return {
        ...
        "features": ["personalized_ai", "print_chat"],
        "endpoints": {
...
async def create_conversation(
    ...
    """Create a new conversation with personalized AI"""
    ...
    try:
        ...
        user_name = data.user_name or user.get('first_name', 'friend')
        
        print(f"👤 Personalizing for: {user_name}")
        
        # Create conversation with user_name stored
        ...
        if detect_emergency(data.initial_message):
            ai_response = f"""🚨 MEDICAL EMERGENCY DETECTED

{user_name}, based on what you've described, this could be a medical emergency.
...
This is not medical advice, but these symptoms require immediate professional evaluation."""
            ...
            system_prompt = build_personalized_prompt(user_name, email)
            
            # Call Claude API with personalized prompt
            print(f"🤖 Calling Claude API with personalized prompt...")
...
async def send_message(
    ...
    """Send a message to existing conversation with personalization"""
    ...
    try:
        ...
        # Get user's name from conversation
        user_name = conversations[conversation_id].get('user_name', 'friend')
        print(f"👤 Continuing personalized conversation with: {user_name}")
        ...
        if detect_emergency(data.content):
            ai_response = f"🚨 MEDICAL EMERGENCY DETECTED\n\n{user_name}, please call 911 immediately or go to the nearest emergency room. Do not wait."
