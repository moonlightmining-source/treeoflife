    ...
    with engine.connect() as conn:
        ...
        if protocol[10]:  # weekly_notes
            ...
            if isinstance(notes_data, dict):
                ...
                possible_keys = [
                    f"week_{current_week_num}",
                    f"Week {current_week_num}",
                    f"week{current_week_num}",
                    str(current_week_num)
                ]
                
                week_note = None
                for key in possible_keys:
                    if key in notes_data and notes_data[key]:
                        note_text = str(notes_data[key]).strip()
                        if note_text:
                            week_note = note_text
                            break
                
                if week_note:
                    lifestyle_changes.insert(0, f"⭐ This Week's Focus: {week_note}")
