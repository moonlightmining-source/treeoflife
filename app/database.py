...
class User(Base):
    ...
    first_name = Column(String, nullable=True)    # ✅ FIXED: Now nullable
    last_name = Column(String, nullable=True)     # ✅ FIXED: Now nullable
    name = Column(String, nullable=True)          # Keep for backward compatibility
    date_of_birth = Column(DateTime, nullable=True)  # ✅ NEW: Birth date field
    phone = Column(String, nullable=True)         # ✅ NEW: Phone number
    address = Column(Text, nullable=True)         # ✅ NEW: Address field
    zip_code = Column(String, nullable=True)      # ✅ NEW: ZIP CODE
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if 'users' in inspector.get_table_names():
                ...
                # ===================================================================
                # DROP NOT NULL CONSTRAINTS (FIX FOR RENDER $6 PLAN - NO SHELL)
                # ===================================================================
                print("  🔧 Removing NOT NULL constraints from user columns...")
                
                # Columns that should be nullable
                nullable_columns = ['first_name', 'last_name', 'name', 'date_of_birth', 'phone', 'address', 'zip_code']
                ...
                for col_name in nullable_columns:
                    if col_name in existing_columns:
                        try:
                            ...
                            print(f"  ✅ Removed NOT NULL from '{col_name}'")
                        except Exception as e:
                            # Column might already be nullable
                            if 'does not exist' not in str(e).lower():
                                print(f"  ℹ️  {col_name}: {e}")
                
                if constraints_dropped > 0:
                    conn.commit()
                    print(f"  🎉 Dropped {constraints_dropped} NOT NULL constraint(s)!")
                else:
