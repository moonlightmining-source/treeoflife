...
def create_user(email: str, password_hash: str, db: Session, **kwargs) -> User:
    """Create a new user"""
    user = User(
        email=email,
...
def update_user(user_id: int, data: dict, db: Session) -> Optional[User]:
    """Update user data"""
...
def create_message(conversation_id: int, role: str, content: str, db: Session) -> Message:
...
def get_messages(conversation_id: int, db: Session) -> List[Message]:
...
def create_health_profile(user_id: int, data: dict, db: Session) -> HealthProfile:
...
def get_health_profile(user_id: int, db: Session) -> Optional[HealthProfile]:
