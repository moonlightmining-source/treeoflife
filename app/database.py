...
def fix_missing_columns():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            # ===================================================================
            # FIX BROKEN HEALTH_PROFILES TABLE (DROP IF ID ISN'T AUTO-INCREMENT)
            # ===================================================================
            if 'health_profiles' in inspector.get_table_names():
                # Check if the ID column has a sequence (autoincrement)
                try:
                    ...
                    # If column_default is NULL, the ID doesn't autoincrement
                    if row and row[0] is None:
                        print("  ⚠️  health_profiles.id is broken (no autoincrement)")
                        print("  🗑️  Dropping health_profiles table to recreate with correct schema...")
                        conn.execute(text("DROP TABLE health_profiles CASCADE;"))
                        conn.commit()
                        print("  ✅ health_profiles table dropped - will be recreated")
                    else:
                        print("  ✅ health_profiles.id has autoincrement - table is OK")
                except Exception as e:
                    print(f"  ℹ️  Could not check health_profiles.id: {e}")
...
def create_conversation(db: Session, user_id: uuid.UUID, conversation_id: str = None) -> Conversation:
...
def update_health_profile(db: Session, user_id: uuid.UUID, profile_data: dict) -> HealthProfile:
